...
const getTextShadowStyle = (outline, shadow) => {
  const parts = [];
  if (outline) {
    parts.push('-1px -1px 0 #000', '1px -1px 0 #000', '-1px 1px 0 #000', '1px 1px 0 #000');
  }
  if (shadow === 'subtle') {
    parts.push('0 1px 2px rgba(0,0,0,0.6)');
  } else if (shadow === 'medium') {
    parts.push('0 2px 4px rgba(0,0,0,0.8)', '0 1px 2px rgba(0,0,0,0.5)');
  } else if (shadow === 'heavy') {
    parts.push('0 4px 8px rgba(0,0,0,0.9)', '0 2px 4px rgba(0,0,0,0.7)');
  }
...
export default function OverlayView() {
  ...
  const [settings, setSettings] = useState(() => {
    ...
    // 3. Load from URL query parameters (which can act as overrides)
    const query = new URLSearchParams(window.location.search);
    const urlSettings = {};
    if (query.has('theme')) urlSettings.theme = query.get('theme');
    if (query.has('chatStyle')) urlSettings.chatStyle = query.get('chatStyle');
    if (query.has('textSize')) urlSettings.textSize = parseInt(query.get('textSize'));
    if (query.has('fontFamily')) urlSettings.fontFamily = query.get('fontFamily');
    if (query.has('showTimestamps')) urlSettings.showTimestamps = query.get('showTimestamps') === 'true';
    if (query.has('showIcons')) urlSettings.showIcons = query.get('showIcons') === 'true';
    if (query.has('showBadges')) urlSettings.showBadges = query.get('showBadges') === 'true';
    if (query.has('showLevelBadges')) urlSettings.showLevelBadges = query.get('showLevelBadges') !== 'false';
    if (query.has('showChannelName')) urlSettings.showChannelName = query.get('showChannelName') === 'true';
    if (query.has('removeAtSymbol')) urlSettings.removeAtSymbol = query.get('removeAtSymbol') === 'true';
    
    const showPics = query.get('showProfilePictures') !== 'false';
    if (query.has('showYoutubeProfilePictures')) urlSettings.showYoutubeProfilePictures = query.get('showYoutubeProfilePictures') !== 'false';
    else if (query.has('showProfilePictures')) urlSettings.showYoutubeProfilePictures = showPics;
    
    if (query.has('showTwitchProfilePictures')) urlSettings.showTwitchProfilePictures = query.get('showTwitchProfilePictures') !== 'false';
    else if (query.has('showProfilePictures')) urlSettings.showTwitchProfilePictures = showPics;
    
    if (query.has('showKickProfilePictures')) urlSettings.showKickProfilePictures = query.get('showKickProfilePictures') !== 'false';
    else if (query.has('showProfilePictures')) urlSettings.showKickProfilePictures = showPics;
    
    if (query.has('hideBotMessages')) urlSettings.hideBotMessages = query.get('hideBotMessages') === 'true';
    if (query.has('randomNameColors')) urlSettings.randomNameColors = query.get('randomNameColors') !== 'false';
    if (query.has('overlayFadeTime')) urlSettings.overlayFadeTime = parseInt(query.get('overlayFadeTime'));
    if (query.has('overlayTextOutline')) urlSettings.overlayTextOutline = query.get('overlayTextOutline') !== 'false';
    if (query.has('overlayTextShadow')) urlSettings.overlayTextShadow = query.get('overlayTextShadow');
    if (query.has('customCss')) urlSettings.overlayCustomCss = query.get('customCss');

    return { ...defaults, ...local, ...urlSettings };
  });

  const query = new URLSearchParams(window.location.search);
  const channelsParam = query.get('channels') || ''; // format: "platform:name,platform:name"
  ...
  const {
    ...
    showChannelName,
    removeAtSymbol,
    showYoutubeProfilePictures,
    showTwitchProfilePictures,
    showKickProfilePictures,
    hideBotMessages,
    randomNameColors,
    overlayFadeTime,
    overlayTextOutline,
    overlayTextShadow,
    overlayCustomCss: customCss
  } = settings;

  // Listen to settings updates from the main dashboard in real-time
  useEffect(() => {
    const handleStorageChange = (e) => {
      if (e.key === 'prochat_settings') {
        try {
          const nextSettings = JSON.parse(e.newValue);
          if (nextSettings) {
            setSettings(prev => ({
              ...prev,
              ...nextSettings
  ...
  const showAvatarForPlatform = (platform) => {
    const norm = platform ? platform.toLowerCase() : '';
    if (norm === 'youtube') return showYoutubeProfilePictures;
    if (norm === 'twitch') return showTwitchProfilePictures;
    if (norm === 'kick') return showKickProfilePictures;
    return false;
  };
  ...
  const renderKickBadge = (badge, msg) => {
    ...
    if (badge.startsWith('level_')) {
      if (!showLevelBadges) return null;
      ...
      if (level < 10) {
        ...
        shapeSvg = (
          <svg width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg" style={{ marginRight: '4px', verticalAlign: 'middle', display: 'inline-block' }}>
            <defs>
              <linearGradient id="levelGreyGradient-overlay" x1="0%" y1="0%" x2="100%" y2="100%">
            ...
            <circle 
              ...
              cy="9" 
              r="7.5" 
              fill="url(#levelGreyGradient-overlay)" 
              stroke="#A4A4A4"
              strokeWidth="0.8"
            />
        ...
        shapeSvg = (
          <svg width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg" style={{ marginRight: '4px', verticalAlign: 'middle', display: 'inline-block' }}>
            <defs>
              <linearGradient id="levelYellowGradient-overlay" x1="0%" y1="0%" x2="100%" y2="100%">
            ...
            <rect 
              ...
              rx="3" 
              ry="3" 
              fill="url(#levelYellowGradient-overlay)" 
              stroke="#C69A00"
              strokeWidth="0.8"
            />
        ...
        shapeSvg = (
          <svg width="18" height="18" viewBox="0 0 18 18" xmlns="http://www.w3.org/2000/svg" style={{ marginRight: '4px', verticalAlign: 'middle', display: 'inline-block' }}>
            <defs>
              <linearGradient id="levelCyanGradient-overlay" x1="0%" y1="0%" x2="100%" y2="100%">
            ...
            <rect 
              ...
              fill="url(#levelCyanGradient-overlay)" 
              stroke="#00B1BC"
              strokeWidth="0.8"
            />
            <text x="9" y="9.5" fontFamily="system-ui, -apple-system, sans-serif" fontSize="9" fontWeight="900" fill="#000" textAnchor="middle" dominantBaseline="central">
        ...
        <span key={badge} className="kick-chatter-badge" title="Founder" style={{ display: 'inline-flex', alignItems: 'center' }}>
          <svg className="kick-svg-element" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg" style={iconStyle}>
            <g clipPath="url(#KickFounderNewClipPath)">
              ...
              <path d="M16 32C24.8366 32 32 24.8366 32 16C32 7.16344 24.8366 0 16 0C7.16344 0 0 7.16344 0 16C0 24.8366 7.16344 32 16 32Z" fill="white" fillOpacity="0.3"></path>
  ...
  const getUsernameColor = (msg) => {
    ...
    if (platform === 'kick') {
      ...
      if (msg.badges && msg.badges.includes('subscriber')) {
        return '#53fc18';
      }
      return msg.color || '#53fc18';
    }
    ...
    if (platform === 'twitch') {
      ...
      return msg.color || '#ff69b4';
    }

    if (randomNameColors && !msg.color) {
      return getDeterministicColor(msg.username);
    }
  ...
  // Parse active channels list
  const activeChannels = channelsParam
    ? channelsParam.split(',').map((item, index) => {
        const [platform, name] = item.split(':');
        return { id: index, platform, name: name.trim() };
      })
    : [];
  ...
  // Inject custom CSS dynamically in document head
  useEffect(() => {
    const styleId = 'obs-custom-css';
  ...
  // Dynamic theme class observer for transparent body background
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const handleNewMessage = (msg) => {
      ...
      setMessages(prev => [...prev, msg].slice(-30)); // limit overlay size to 30 items
      
      // Auto-fade timer: only trigger if overlayFadeTime > 0
      if (overlayFadeTime > 0) {
        setTimeout(() => {
          setFadingMsgIds(prev => {
        ...
        }, (overlayFadeTime + 1) * 1000);
      }
    };

    // Initialize Twitch client
    let twitchClient = null;
    const twitchChannels = activeChannels.filter(ch => ch.platform === 'twitch');
    if (twitchChannels.length > 0) {
      twitchClient = new TwitchChatClient(handleNewMessage, () => {});
      twitchClient.connect();
      twitchChannels.forEach(ch => twitchClient.join(ch.name));
    }

    // Initialize Kick client
    let kickClient = null;
    const kickChannels = activeChannels.filter(ch => ch.platform === 'kick');
    if (kickChannels.length > 0) {
      kickClient = new KickChatClient(handleNewMessage, () => {});
      kickClient.connect();
      kickChannels.forEach(ch => kickClient.join(ch.name));
    }

    // Initialize YouTube client
    let youtubeClient = null;
    const youtubeChannels = activeChannels.filter(ch => ch.platform === 'youtube');
    if (youtubeChannels.length > 0) {
      ...
      youtubeChannels.forEach(ch => youtubeClient.join(ch.name));
    }

    // Initialize Simulator for simulated streams
    let simulator = null;
    const simulatedChannels = activeChannels.filter(ch => 
      ch.platform === 'tiktok' || ch.platform === 'rumble' || ch.platform === 'x'
    );
    if (simulatedChannels.length > 0) {
      simulator = new ChatSimulator(handleNewMessage);
      simulator.start(simulatedChannels);
    }

    return () => {
      if (twitchClient) twitchClient.disconnect();
      if (kickClient) kickClient.disconnect();
  ...
  return (
    <div 
      ...
      style={{ 
        ...
        fontFamily: fontFamily === 'inherit' ? 'inherit' : fontFamily || 'inherit'
      }}
    >
      <div className={`feed-messages style-${chatStyle}`} style={{ background: 'transparent', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
        {messages.map(msg => {
          ...
          if (msg.isSystemEvent) {
            const shadowStyle = getTextShadowStyle(overlayTextOutline, overlayTextShadow);
            return (
              <div 
                key={msg.id} 
                className={`system-event-row ${msg.platform}-${msg.eventType} ${isFading ? 'fading' : ''}`}
                style={{ opacity: isFading ? 0 : 1, transition: 'opacity 0.8s ease-out' }}
              >
                <div className="event-header">
                  ...
                  <span style={{ color: msg.color, textShadow: shadowStyle }} className="msg-username">{getFormattedName(msg)}</span>
                  <span style={{ color: 'var(--text-muted)', textShadow: shadowStyle }}>{msg.text}</span>
                </div>
                {msg.eventDetails && msg.eventDetails.amount && (
                  <div className="event-details" style={{ textShadow: shadowStyle }}>{msg.eventDetails.amount}</div>
                )}
              </div>
            );
          }

          const contentParts = msg.parts || parseMessageContent(msg.text);
            ...
            >
              ...
              {msg.repliedTo && (
                <div className="chat-reply-thread-header">
                  ...
                  <span className="reply-thread-text" style={{ textShadow: getTextShadowStyle(overlayTextOutline, overlayTextShadow) }}>
                    Replying to <span className="reply-thread-user">{getFormattedName(msg.repliedTo)}</span>: <span className="reply-thread-body">{msg.repliedTo.text}</span>
                  </span>
                </div>
              )}

              {/* 3. Left Metadata (Timestamp & Platform Icon) */}
              {(showTimestamps || showIcons) && (
                <div className="chat-message-meta-left" style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: '4px', flexShrink: 0, height: '1.5em' }}>
                  ...
                  {showIcons && (
                    <span className="msg-platform-icon" title={msg.platform} style={{ display: 'inline-flex', alignItems: 'center' }}>
                      <PlatformLogo platform={msg.platform} isShorts={msg.isShorts} size={14} />
                    </span>
                  )}
                </div>
              ...
              <div className="chat-message-main-row" style={{ display: 'flex', flexDirection: 'row', alignItems: 'flex-start', gap: '4px', width: '100%' }}>
                ...
                <div className="message-body-inline">
                  {msg.platform !== 'youtube' && showBadges && msg.badges && (msg.platform === 'kick' ? sortKickBadges(msg.badges) : msg.badges).map(badge => {
                    ...
                    if (badge === 'subscriber' || badge === 'member') {
                      return (
                        <span key={badge} style={{ display: 'inline-flex', alignItems: 'center', verticalAlign: 'middle', marginRight: '4px' }} title={badge === 'member' ? 'Member' : 'Subscriber'}>
                          {msg.platform === 'twitch' ? (
                            <TwitchDefaultSubscriberBadge size={14} />
                          ) : (
                    ...
                    return (
                      <span key={badge} className={`msg-badge ${badge} platform-${msg.platform}`} style={{ marginRight: 4 }}>
                        {displayChar}
                      </span>
                    );
                  })}
