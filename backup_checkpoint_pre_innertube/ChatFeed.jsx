...
export default function ChatFeed({ 
  ...
  const visibleMessages = useMemo(() => messages.filter(msg => {
    const u1 = (msg.username || '').replace(/^@+/, '').trim().toLowerCase();
    const u2 = (msg.displayName || '').replace(/^@+/, '').trim().toLowerCase();
    const u3 = (msg.author || '').replace(/^@+/, '').trim().toLowerCase();
    const cId = (msg.channelId || msg.authorChannelId || msg.userId || '').toLowerCase();
            ...
            (() => {
              const msgElements = tabFilteredMessages.map((msg, idx) => {
                ...
                if (isDefaultAvatar(resolvedAvatar)) {
                  const cleanUser = (msg.username || '').toLowerCase().replace(/^@+/, '');
                  const cleanDisplay = (msg.displayName || '').toLowerCase().replace(/^@+/, '');
                  const cleanChannel = (msg.channel || '').toLowerCase().replace(/^@+/, '');
                  const creatorAvatar = user?.avatarUrl || (typeof user?.avatar === 'string' && user?.avatar.startsWith('http') ? user.avatar : null);
                  
                  resolvedAvatar = GLOBAL_AVATAR_CACHE.get(cleanUser) || 
                                   GLOBAL_AVATAR_CACHE.get(cleanDisplay) || 
                                   GLOBAL_AVATAR_CACHE.get(cleanChannel) || 
                                   creatorAvatar || 
                                   null;
                }
                const isDefault = isDefaultAvatar(resolvedAvatar);
                const avatarUrl = isDefault ? getDefaultAvatar(msg.platform, msg.username, msg.userId) : proxifyAvatarUrl(resolvedAvatar);
      ...
      {activeMenuId && menuPos && menuPos.msg && typeof window !== 'undefined' && createPortal(
        ...
        >
          ...
          {menuPos.msg.platform === 'youtube' && (
            ...
            >
              {menuPos.msg.badges && menuPos.msg.badges.includes('moderator') ? (
                <>
