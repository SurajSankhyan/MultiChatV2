...
export async function POST(request: Request) {
  try {
    ...
    // 4. Try 0-Quota Innertube Engine First (Alternative 1)
    if (isValidLiveChatId(activeChatId)) {
      try {
        ...
        if (innertubeRes.ok) {
          ...
          if (innertubeData.success && innertubeData.engine === 'innertube') {
            console.log(`[YouTube API Route] Action "${action || 'send'}" successfully processed via 0-Quota Innertube Engine!`);
            return NextResponse.json(innertubeData);
          }
        }
      } catch (e: any) {
        console.warn('[YouTube API Route] Innertube direct dispatch note:', e.message);
      }
    }

    // 5. Perform Action via YouTube Data API Fallback
    if (action === 'delete') {
      if (!messageId) {
